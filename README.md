...
#### 동작 과정
1. 사용자가 웹사이트에 방문하면 브라우저가 서버에 컨텐츠 요청
2. 서버는 빈 HTML을 응답
3. 브라우저가 연결된 JS링크를 통해 서버로부터 JS파일 다운로드
4. JS를 통해 동적으로 페이지를 만든 후, 브라우저에 출력
#### 장점
- 초기 페이지 로딩이 빠르고 페이지 전환이 자연스러움
- 서버에 자주 요청하지 않으니 서버 부담이 적음
- 클릭, 드래그 같은 다양한 상호작용 구현 가능
#### 단점
- 처음에 JS를 모두 받아야 해서 시간이 오래 걸림
- 검색 엔진이 내용을 읽기 어려움
- JS가 꺼져있는 환경에서는 아무것도 보이지 않음
