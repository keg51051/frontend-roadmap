...
- 초기 로딩 후 페이지 내에서 화면 일부만 동적으로 바뀜 (전체 새로고침 X)
- React, Vue 같은 최신 프레임워크에서 주로 사용

### MPA (Multi Page Application)
- 페이지 이동 시마다 서버에서 새로운 HTML을 받아 전체 페이지를 렌더링
- 전통적인 웹 방식, SEO 친화적인 구조

### CSR (Client Side Rendering)

#### 동작 과정
1. 사용자가 웹사이트에 방문하면 서버가 빈 HTML을 응답 
2. 브라우저가 JS 파일을 다운로드 받고 실행하여 화면을 동적으로 구성
3. 이후 모든 사용자 인터랙션과 화면 전환이 클라이언트에서 처리

#### 장점
- 초기 페이지 로딩이 빠르고 페이지 전환이 자연스러움
- 렌더링을 클라이언트가 담당하여 서버 부하가 적음
- 클릭, 드래그 같은 다양한 상호작용에 유리

#### 단점
- 처음에 JS를 모두 받아야 해서 시간이 오래 걸림
- SEO에 불리 (초기 HTML 컨텐츠 부족)
- JS가 꺼져있는 환경에서는 동작하지 않음

### SSR (Server Side Rendering)

#### 동작 과정
1. 사용자(브라우저)가 페이지 요청
2. 서버가 필요한 데이터를 가져와 HTML을 "완성된 상태"로 렌더링해서 전송
3. 브라우저는 그 HTML을 즉시 렌더링하고 화면에 표시
4. 이후 JS 번들이 실행되면서 하이드레이션(서버 생성 HTML에 JS 기능을 붙이는 과정)을 수행
5. CSR처럼 클라이언트가 상태와 라우팅을 처리

#### 장점
- JS를 모두 다운로드 + 실행하기 전에 컨텐츠가 HTML로 보이므로 첫 화면 표시가 빠름
- 크롤러가 이미 렌더링된 HTML을 바로 읽을 수 있어서 SEO에 유리
- 메타태그, OG 태그 등을 서버에서 채워 내려주기 때문에 미리보기가 정확하게 나옴

#### 단점
- 요청마다 렌더링과 데이터 페칭이 일어나므로 트래픽이 많으면 서버 부하가 증가
- 데이터 페칭을 서버 기준으로 설계하고, 캐싱, 에러 핸들링, 환경 분기를 신경 써야돼 구현이 어려움
- TTFB(Time To First Byte) 첫 조회 바이트가 늦어져 체감 속도가 나쁠 수 있음

### SPA, MPA, SSR, CSR
- SPA와 MPA는 아키텍처 구분, SPA는 단일 페이지, MPA는 다중 페이지 구조
- SSR과 CSR은 렌더링 수행 위치의 차이, SSR은 서버, CSR은 클라이언트
- 일반적으로 SPA - CSR, MPA - SSR 이지만 혼용도 가능 (예: Next.js는 MPA 구조에서 SSR 사용)

### CSR, SSR 차이
| 구분         | CSR (Client Side Rendering)              | SSR (Server Side Rendering)                   |
|--------------|----------------------------------------|----------------------------------------------|
| **렌더링 위치**  | 브라우저에서 JS 실행 후 화면 렌더링      | 서버가 완성된 HTML을 만들어서 보내고 브라우저가 렌더링 |
| **초기 속도**    | JS 다운로드 후 렌더링, 초기 진입 느릴 수 있음 | 서버가 HTML을 보내서 첫 화면이 빠르게 보임          |
| **이후 동작**    | 클라이언트에서 상태/라우팅 처리           | 하이드레이션 후 클라이언트에서 상태/라우팅 처리       |
| **SEO**          | JS 실행 전에는 내용 없음, SEO 불리함       | 완성된 HTML에 콘텐츠 포함, SEO에 유리                |
| **서버 부하**    | 상대적으로 적음                         | 서버에서 렌더링 처리하므로 부하와 비용 증가            |
